...
##############################################################################
    ...
    def add_dates(self, self_day2): 
        new_day = self.day +self_day2.day                # مقادیر روزها در تاریخ خط 61 و 62
        new_month = self.month + self_day2.month         # مقادیر ماه ها در تاریخ خط 61 و 62
        new_year = self.year + self_day2.year            # مقادیر سال ها در تاریخ خط 61 و 62
    ...
    def isvalidDate(self):                                # چک کردن اعتبار
        if self.year not in range(1 , 9999):              # اگر مقدار سال بین 1 تا 9999 نبود
            return True                                  # print false  بگو غلطه
            ...
            return False                                   #print True  بگو درسته
...
while True:
    ...
    if user_input=="1":                                               #عدد یک بصورت استرینگ تعریف شده است چون هرعددی توی ورود کاربر رشته میباشد .
        ...
        date2 = Date(d, e, f)
        for i in range(3):
            User_Choice = int(input('If u Want To add Type 1 elif Sub Type 2 elif Month Name Type 3  elif IsvalidDate Type 4 elsif Type Anykey :'))#درخواست از کاربر برای انتخاب عملیات
            if User_Choice==1:
